...
class sync_grades extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        if ($tools = $DB->get_records_sql($sql, array('status' => ENROL_INSTANCE_ENABLED))) {
                ...
                if ($tool->lastgradesynctime + $synctime < $timenow) {
                    ...
                    if ($users = $DB->get_records('enrol_lti_users', array('toolid' => $tool->id))) {
                        foreach ($users as $user) {
                            $mtracecontent = "for the user '$user->userid' in the tool '$tool->id' for the course '$tool->courseid'";
                            ...
                            if ($context = \context::instance_by_id($tool->contextid)) {
                                ...
                                if ($sendgrades) {
                                    ...
                                    if (strpos(strtolower($response), 'success') !== false) {
                                        ...
                                        mtrace("Failed - The grade '$floatgrade' $mtracecontent failed to send to the remote system.");
                                        $errorcount = $errorcount + 1;
                                    }
                                } else {
                                    mtrace("Not sent - The grade $mtracecontent was intentionally not sent to the remote system.");
                                    $errorcount = $errorcount + 1;
                                }
                            } else {
                    ...
                    mtrace("Completed - Synced grades for tool '$tool->id' in the course '$tool->courseid'. " .
                        "Processed $usercount users; sent $sendcount grades and did not send $errorcount grades.");
                    $DB->set_field('enrol_lti_tools', 'lastgradesynctime', $timenow, array('id' => $tool->id));
                } else {
                    $last = format_time((time() - $tool->lastgradesynctime));
                    mtrace("Skipping grade sync for shared tool '$tool->id' for the course '$tool->courseid' as " .
                        "it was synchronised $last ago.");
                }
            }
        }
    }
