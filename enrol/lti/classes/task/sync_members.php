...
class sync_members extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        if ($tools = enrol_lti_get_lti_tools(array('status' => ENROL_INSTANCE_ENABLED, 'membersync' => 1))) {
            ...
            foreach ($tools as $tool) {
                if ($tool->lastmembersynctime + $synctime < $timenow) {
                    ...
                    if ($ltiusers = $DB->get_records('enrol_lti_users', array('toolid' => $tool->id), 'lastaccess DESC')) {
                                ...
                                if (!empty($data->statusinfo)) {
                                    if (strpos(strtolower($data->statusinfo->codemajor), 'success') !== false) {
                                        ...
                                        foreach ($members as $member) {
                                            // Set the user data.
                                            $user = new \stdClass();
                                            $user->username = enrol_lti_create_username($user->consumerkey, $member->user_id);
                                            ...
                                            if (!$dbuser = $DB->get_record('user', array('username' => $user->username,
                                                'deleted' => 0))) {
                                                if ($tool->membersyncmode == ENROL_LTI_MEMBER_SYNC_ENROL_AND_UNENROL ||
                                                    $tool->membersyncmode == ENROL_LTI_MEMBER_SYNC_ENROL_NEW) {
                                                    ...
                                                    if (empty($user->email)) {
                                                        $user->email = $user->username .  "@ltiuser.com";
                                                    }
                                            ...
                                            if ($tool->membersyncmode == ENROL_LTI_MEMBER_SYNC_ENROL_AND_UNENROL ||
                                                ...
                                                enrol_lti_enrol_user($tool, $user->id);
                                            }
                                        }
                                    } else {
                        ...
                        if ($tool->membersyncmode == ENROL_LTI_MEMBER_SYNC_ENROL_AND_UNENROL ||
                            $tool->membersyncmode == ENROL_LTI_MEMBER_SYNC_UNENROL_MISSING) {
                    ...
                    mtrace("Completed - Synced members for tool '$tool->id' in the course '$tool->courseid'. " .
                         "Processed $usercount users; synchronised $sendcount members.\");");
                    mtrace("");
                } else {
