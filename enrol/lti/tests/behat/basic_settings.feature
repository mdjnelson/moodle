...
Feature: Check that settings are adhered to when creating an enrolment plugin
  ...
  Scenario: As an admin set site-wide settings for the enrolment plugin and ensure they are used
    ...
    Then the field "User profile update" matches value "Never"
    And the field "Email display" matches value "Allow everyone to see my email address"
    ...
    And the field "User profile update" matches value "Always"
