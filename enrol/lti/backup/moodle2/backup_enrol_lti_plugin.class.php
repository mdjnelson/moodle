...
class backup_enrol_lti_plugin extends backup_enrol_plugin {
    ...
    public function define_enrol_plugin_structure() {
        ...
        $tool = new backup_nested_element('tool', array('id'), array(
            'enrolid', 'contextid', 'userprofileupdate', 'institution', 'lang', 'timezone', 'maxenrolled',
            'maildisplay', 'city', 'country', 'gradesync', 'gradesynccompletion', 'lastgradesynctime', 'membersync',
            'lastmembersynctime', 'roleinstructor', 'rolelearner', 'secret', 'timecreated', 'timemodified'));
        ...
        if ($this->task->get_setting_value('users')) {
            $user->set_source_table('enrol_lti_users',
                array('toolid' => backup::VAR_PARENTID));
        }
    }
}
