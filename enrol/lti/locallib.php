 ...
 *
 * @param \BLTI $context The LTI context
 * @param stdClass $tool The tool object
 * @return stdClass the user record
 */
function enrol_lti_get_user_details($context, $tool) {
    global $CFG;

    $user = new stdClass();

    if (isset($context->info['lis_person_name_given'])) {
        $user->firstname = $context->info['lis_person_name_given'];
    } else {
        $user->firstname = $context->info['user_id'];
    }

    if (isset($context->info['lis_person_name_family'])) {
        $user->lastname = $context->info['lis_person_name_family'];
    } else {
        $user->lastname = $context->info['context_id'];
    }
