...
class core_calendar_container_testcase extends advanced_testcase {
    ...
    function test_event_factory_with_completion_related_event() {
        ...
        $eventinstance = $factory->create_instance($event);
        $this->assertInstanceOf(event_interface::class, $eventinstance);

        $CFG->enablecompletion = false;

        $event = $factory->create_instance($event);
        // The event should be invisible since we have disabled completion.
        $this->assertNull($event);
    }
