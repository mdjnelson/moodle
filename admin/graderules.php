...
$PAGE->set_url('/admin/graderules.php');
...
switch ($action) {
    case 'disable':
        if ($plugins[$name]->is_enabled()) {
            set_config('disabled', 1, 'graderule_'. $name);
        }
        break;
    case 'enable':
        if (!$plugins[$name]->is_enabled()) {
            unset_config('disabled', 'graderule_'. $name);
        }
        break;
    case 'up':
        if ($sortorder[$name]) {
            ...
            set_config('graderule_plugins_sortorder', implode(',', $seq));
        }
        break;
    case 'down':
