...
class auth_db_events_testcase extends advanced_testcase {
    ...
    /**
     * Tests that the locations in the auth_db API that update the user trigger a user_updated event.
     */
    public function test_user_updated() {
        ...
        // We don't want to duplicate any unnecessary code.
        ...
        // deal with the record here. In this case it will unsuspend the user.
        $user = new stdClass();
        $user->name = 'sirmark';
        $user->pass = 'TheLegend';
        ...
        // Run sync_users and capture the update_user event.
        ...
        // Test the user update event was triggered (by calling the function user_update_user) - no need
        // to test the other details of the event as that is done extensively in user/tests/userlib_test.php
        $this->assertInstanceOf('\core\event\user_updated', $event);

        // Run sync_users and capture the update_user event.
        $sink = $this->redirectEvents();
        $auth->update_user_record('SirMark');
        $events = $sink->get_events();
        $sink->close();
        $event = array_pop($events);
