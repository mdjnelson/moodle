...
function user_create_user($user, $updatepassword = true) {
    ...
    if ($DB->record_exists('user', array('username' => $user->username, 'mnethostid' => $CFG->mnet_localhost_id))) {
        throw new moodle_exception('Username already exists: ' . $user->username);
    }

    // Check that the email is valid and does not already exist.
    if (!validate_email($user->email)) {
        throw new moodle_exception('Email address is invalid: ' . $user->email);
    } else if ($DB->record_exists('user', array('email' => $user->email, 'mnethostid' => $user->mnethostid))) {
        throw new moodle_exception('Email address already exists: ' . $user->email);
    }
    ...
    if (!empty($user->auth)) {
        ...
        if ($user->auth === 'mnet') {
            throw new moodle_exception('Invalid authentication type: mnet requires a mnethostid in order to work');
        } else if (empty($availableauths[$user->auth])) {
            throw new moodle_exception('Invalid authentication type: ' . $user->auth);
        }
    }
    ...
    if (!empty($user->lang)) {
        ...
        if (empty($availablelangs[$user->lang])) {
            throw new moodle_exception('Invalid language code: ' . $user->lang);
        }
    }
    ...
    if (!empty($user->theme)) {
        ...
        if (empty($availablethemes[$user->theme])) {
            throw new moodle_exception('Invalid theme: ' . $user->theme);
        }
    }
