...
class mod_quiz_events_testcase extends advanced_testcase {
    ...
    /**
     * Test the questions updated event.
     *
     * There is no external API for updating questions, so the unit test will simply
     * create and trigger the event and ensure the event data is returned as expected.
     */
    public function test_questions_updated() {
        $this->resetAfterTest();
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_quiz\event\questions_updated', $event);
    ...
    public function test_report_viewed() {
        ...
        $params = array(
            ...
            'other' => array('reportname' => 'overview',
                             'quizid' => $quiz->id)
        );
    ...
    /**
     * Test the attempt closed event.
     *
     * There is no external API for closing an attempt, so the unit test will simply
     * create and trigger the event and ensure the event data is returned as expected.
     */
    public function test_attempt_closed() {
        $this->resetAfterTest();
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_quiz\event\attempt_closed', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'quiz', 'close attempt', 'review.php?attempt=1', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
    }
     ...
     * create and trigger the event and ensure the event data is returned as expected.
     */
    public function test_override_updated() {
        $this->resetAfterTest();
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_quiz\event\override_updated', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
    ...
    public function test_attempt_continued() {
        ...
        $event = \mod_quiz\event\attempt_continued::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_quiz\event\attempt_continued', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
     ...
     */
    public function test_attempt_previewed() {
        list($quizobj, $quba, $attempt) = $this->prepare_quiz_data();
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_quiz\event\attempt_previewed', $event);
        $this->assertEquals(context_module::instance($quizobj->get_cmid()), $event->get_context());
     ...
     *
     * There is no external API for manually grading questions, so the unit test will simply
     * create and trigger the event and ensure the event data is returned as expected.
     */
    public function test_question_manually_graded() {
        ...
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'quiz', 'manualgrade', 'comment.php?attempt=1&slot=2', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
    }
}
