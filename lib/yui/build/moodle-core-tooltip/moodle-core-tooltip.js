...
Y.extend(TOOLTIP, M.core.dialogue, {
    ...
    /**
     * Display the custom tooltip for the clicked link.
     *
     * The anchor for the clicked link is used.
     *
     * @method display_panel
    ...
    display_custom_panel: function(e) {
        ...
        url = Y.bind(this.get('urlmodifier'), this, clickedlink.get('href'))();

        // Get the title and text from the url.
        var params = Y.use('querystring-parse', function(Y) {
            var params = Y.QueryString.parse(url);
            var title = 'title';
            var text = 'text';
