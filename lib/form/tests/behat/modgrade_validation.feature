...
Feature: Using the activity grade form element
  ...
  Scenario: Being able to change the grade type, scale and maximum points when there are no grades
    ...
    And I press "Save and display"
    And I should not see "You can not change the scale as grades already exist for this item"
    And I click on "Edit settings" "link" in the "Administration" "block"
    And I expand all fieldsets
    And I set the field "scale[modgrade_type]" to "Point"
    And I set the field "Maximum points" to "50"
    And I press "Save and display"
    And I should not see "You can not change the maximum points when grades already exist for a rated item"

  Scenario: Attempting to change grade type when grades already exist
