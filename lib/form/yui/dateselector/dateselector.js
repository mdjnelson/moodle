YUI.add('moodle-form-dateselector', function(Y) {
    ...
    CALENDAR.prototype = {
        ...
        initializer : function(config) {
            // Check if we are passing the variables already - means we are processing a date selector in a group.
            if (config.dayselect &&  config.monthselect && config.yearselect && config.calendarimage) {
                // Set the class variables.
                this.dayselect = config.dayselect;
                this.monthselect = config.monthselect;
                this.yearselect = config.yearselect;
                this.calendarimage = config.calendarimage;
                this.enablecheckbox = config.enablecheckbox;

                this.dayselect.after('change', this.handle_select_change, this);
                this.monthselect.after('change', this.handle_select_change, this);
                this.yearselect.after('change', this.handle_select_change, this);
                this.calendarimage.on('click', this.focus_event, this);
                // Check if there is an enabled checkbox.
                if (this.enablecheckbox) {
                    this.enablecheckbox.on('click', this.toggle_calendar_image, this);
                }
            } else {
                ...
                inputs.each(function(node) {
                    ...
                    } else if (node.get('name').match(/\[enabled]/)) { // The enabled checkbox field.
        ...
        claim_calendar : function() {
            ...
            this.set_date_from_selects();
            M.form.dateselector.currentowner = this.dayselect;
    ...
    M.form.dateselector = {
        ...
        init_date_selectors : function(config) {
            ...
            Y.all('fieldset.fdate_selector_in_group').each(function(node) {
                // Ok, we are in a group that may have multiple date_selector or date_time_selector elements.
                // This will ensure that the JavaScript is set to trigger on each of these. Note: not all the
                // selects will be related to a date_selector or date_time_selector element so we must check.
                var selects = node.all('select');
                selects.each(function(selnode) {
                    // Must be a date_selector element.
                    if (selnode.get('name').match(/\[year]/)) {
                        // Get the name of the date_selector form element.
                        var name = selnode.get('name');
                        name = name.substring(0, name.length - 6);
                        // Ok, we have the name, now get the other elements.
                        config.dayselect = node.all('*[name="' + name + '[day]"]').item(0);
                        config.monthselect = node.all('*[name="' + name + '[month]"]').item(0);
                        config.yearselect = node.all('*[name="' + name + '[year]"]').item(0);
                        config.calendarimage = node.all('*[name="' + name + '[calendar]"]').item(0);
                        config.enablecheckbox = node.all('*[name="' + name + '[enabled]"]').item(0);
        ...
        document_click : function(e) {
            if (this.currentowner) {
                if (this.currentowner) {
                    ...
                    CALENDAR.prototype.release_calendar();
                }
            }
        }
    }
