...
class manager {
    ...
    private static function start_session(bool $requireslock) {
        ...
        try {
            ...
            if (!$requireslock) {
                $seshtohash = (array) $_SESSION['SESSION'];
                unset($seshtohash['cachestore_session']);
                self::$seshhash = md5(serialize($seshtohash));
            }
    ...
    public static function write_close() {
        ...
        if (self::$sessionactive) {
            ...
            if (!self::$handler->requires_write_lock()) {
                ...
                $newseshhash = (array) $_SESSION['SESSION'];
                unset($newseshhash['cachestore_session']);
                $newseshhash = md5(serialize($newseshhash));
